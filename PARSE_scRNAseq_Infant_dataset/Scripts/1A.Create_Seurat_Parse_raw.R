...
save_path <- file.path(save_dir, "BCG_PARSE_unfiltered_merged.rds")
...
# Keep only the sample folders, drop "process" and "all-sample"
...
for (sample_path in sample_dirs) {
  ...
  stim      <- ifelse(length(parts) > 2, paste(parts[3:length(parts)], collapse = "_"), NA_character_)
  # e.g. "media_treated" (or whatever pattern you have)
  
  dge_path <- file.path(sample_path, "DGE_unfiltered")
  
  if (!file.exists(file.path(dge_path, "count_matrix.mtx"))) {
    warning(paste("Skipping", sample_name, "- missing DGE_unfiltered data at", dge_path))
    next
  }
  ...
  # Store in list (use sample_name as key to avoid collisions)
...
if (length(seurat_list) == 0) {
  stop("No Seurat objects were created. Check directory structure and DGE_unfiltered paths.")
}

# Start with the first object, merge the rest
seu_merged <- Reduce(function(x, y) merge(x, y), seurat_list)
seu_merged@project.name <- "BCG_Parse_Merged_Unfiltered"
...
# Hemoglobin content (HB*, excluding HBP if present)
